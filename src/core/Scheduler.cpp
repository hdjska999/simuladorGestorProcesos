...
void Scheduler::addProcess(Process p) { processes.push_back(p); }
...
void Scheduler::printTable() {
  ...
  for (int i = 0; i < processes.size(); i++) {
    if (processes[i].state == RUNNING || processes[i].state == READY ||
  ...
  cout << "  "
          ...
          "---+"
       << endl;
  cout << "  |   PID   | PRIO   |  ESTADO  | MEM   | BURST    | RESTANTE | "
          "ESPERA   |"
       << endl;
  cout << "  "
          "+---------+--------+----------+-------+----------+----------+-------"
  ...
  for (int i = 0; i < processes.size(); i++) {
    ...
    cout << marker << "| " << setw(7) << processes[i].pid << " "
...
void Scheduler::printStats() {
  ...
  cout << "  +=========================================================+"
       << endl;
  cout << "  | PID   | BURST   | ESPERA  | RETORNO   | MOTIVO          |"
       << endl;
  cout << "  +-------+---------+---------+-----------+-----------------+"
       << endl;
  ...
  for (int i = 0; i < processes.size(); i++) {
    ...
    cout << "  | " << setw(5) << processes[i].pid << " | " << setw(5)
         << processes[i].burstTime << " t"
         << " | " << setw(5) << processes[i].waitTime << " t"
         << " | " << setw(7) << processes[i].turnaroundTime << " t"
         << " | " << setw(15) << processes[i].exitReason << " |" << endl;
  ...
  if (counted > 0) {
    cout << "  +-------+---------+---------+-----------+-----------------+"
         << endl;
    cout << "  | Tiempo prom. espera  : " << fixed << setprecision(2)
         << (totalWait / counted) << " ticks" << endl;
    cout << "  | Tiempo prom. retorno : " << fixed << setprecision(2)
         << (totalTurn / counted) << " ticks" << endl;
  }
  cout << "  +=========================================================+"
       << endl;
}
...
void Scheduler::run(ResourceManager &rm) {
  cout << "\n  *** Iniciando simulacion con " << processes.size()
       << " procesos ***\n";
...
void Scheduler::runFCFS(ResourceManager &rm) {
  ...
  // 2: Ejecutar los procesos uno por uno en orden
  for (int i = 0; i < processes.size(); i++) {
...
void Scheduler::runSJF(ResourceManager &rm) {
  ...
  while (procesosTerminados < totalProcesos) {
    ...
    processes[indexElegido].turnaroundTime =
        processes[indexElegido].finishTime -
        processes[indexElegido].arrivalTime;
...
void Scheduler::runRoundRobin(ResourceManager &rm) {
  ...
  while (!colaListos.empty()) {
    ...
    while (processes[index].quantumUsed < quantum &&
...
void Scheduler::printEventLog() {
  ...
  for (int i = 0; i < eventLog.size(); i++) {
    cout << "  | [T=" << setw(3) << eventLog[i].tick << "] " << left << setw(50)
         << eventLog[i].event << " |" << endl;
  }
  cout << "  +=========================================================+"
       << endl;
}
